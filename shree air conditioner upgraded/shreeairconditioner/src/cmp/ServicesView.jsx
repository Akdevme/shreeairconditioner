  ...
  return (
    <div className="space-y-8 pb-8 pt-20 sm:pb-10 sm:pt-24">
      <section className="rounded-[2rem] border border-sky-100 bg-[linear-gradient(-225deg,#FFFEFF_0%,#D7FFFE_100%)] p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-8 lg:p-10">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-sky-700" style={{ fontFamily: "'Bruno Ace SC', sans-serif" }}>Services</p>
        <h1 className="mt-3 text-4xl font-black tracking-[-0.06em] text-[#658BC8] sm:text-5xl" style={{ fontFamily: "'trotes', sans-serif" }}>Appliance care with transparent pricing</h1>
        ...
        {services.map((group) => {
          ...
          return (
            <div key={group.category} className="rounded-[1.75rem] border border-slate-200 bg-[linear-gradient(-225deg,#FFFEFF_0%,#D7FFFE_100%)] p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-md">
              ...
              <h2 className="mt-5 text-2xl font-bold text-[#658BC8]" style={{ fontFamily: "'ketika', sans-serif" }}>
                {group.category}
              </h2>
                ...
                {group.items.map((item) => (
                  <div key={item.name} className="rounded-2xl border border-slate-200 bg-[linear-gradient(135deg,#f5f7fa_0%,#c3cfe2_100%)] p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-base font-semibold text-slate-900" style={{ fontFamily: "'ketika', sans-serif" }}>{item.name}</p>
                        {item.note && <p className="mt-1 text-xs text-sky-700">{item.note}</p>}
                      </div>
                      <span className="whitespace-nowrap text-sm font-black text-sky-700" style={{ fontFamily: "'Recharge', sans-serif" }} >{item.price}</span>
                    </div>
                  </div>
                ))}
              </div>
